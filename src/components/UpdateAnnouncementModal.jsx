...
export default function UpdateAnnouncementModal({ isOpen, onClose, announcement, refreshAnnouncements }) {
    ...
    const [formData, setFormData] = useState({
        title: '', summary: '', is_active: false, category: '', application_deadline: '',
        target_audience: '', application_limitations: '', submission_method: '',
        external_urls: [{ url: '' }]
    });
    ...
    useEffect(() => {
        if (isOpen && announcement) {
            ...
            try {
                ...
                else if (typeof announcement.external_urls === 'string' && announcement.external_urls.startsWith('http')) { urls = [{ url: announcement.external_urls }]; }
            } catch (e) {
                if (typeof announcement.external_urls === 'string' && announcement.external_urls.startsWith('http')) { urls = [{ url: announcement.external_urls }]; }
            }
            setFormData({
                title: announcement.title || '', summary: announcement.summary || '', is_active: announcement.is_active,
                category: announcement.category || '', application_deadline: announcement.application_deadline || '',
                target_audience: announcement.target_audience || '', application_limitations: announcement.application_limitations || '',
                submission_method: announcement.submission_method || '', external_urls: urls
            });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const { data: updated, error } = await supabase.from('announcements').update({
                title: formData.title, summary: formData.summary, is_active: formData.is_active,
                category: formData.category, application_deadline: formData.application_deadline || null,
                target_audience: formData.target_audience, application_limitations: formData.application_limitations,
                submission_method: formData.submission_method, external_urls: JSON.stringify(finalUrls)
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex justify-center items-center md:items-end p-4 md:pb-8"
                        onClick={onClose}>
                        <motion.div
                        ...
                        >
                            ...
                            <div className="flex-grow p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-y-auto">
                                ...
                                <div className="lg:col-span-1 space-y-4 lg:pr-4 lg:pl-1 lg:overflow-y-auto">
                                    ...
                                    <div><label htmlFor="target_audience" className="block text-sm font-semibold text-gray-700 mb-1.5">適用對象</label><textarea id="target_audience" name="target_audience" className={`${inputStyles} min-h-[100px]`} rows={4} value={formData.target_audience} onChange={handleChange} /></div>
                                    <div><label htmlFor="application_limitations" className="block text-sm font-semibold text-gray-700 mb-1.5">申請限制</label><textarea id="application_limitations" name="application_limitations" className={`${inputStyles} min-h-[100px]`} rows={4} value={formData.application_limitations} onChange={handleChange} /></div>
                                </div>

                                <div className="lg:col-span-1 flex flex-col">
                                    <label htmlFor="summary" className="block text-sm font-semibold text-gray-700 mb-1.5 flex-shrink-0">公告摘要 (必填)</label>
                                    <div className="relative min-h-[256px] lg:min-h-0 lg:flex-1">
                                        <QuillEditor value={formData.summary} onChange={handleSummaryChange} disabled={isSaving} />
                                    </div>
                                </div>

                                <div className="lg:col-span-1 p-4 bg-slate-500/5 rounded-lg border overflow-y-auto">
